<template>
  <div v-if="editor" class="ddtr" :class="{ dense }">
    <VThemeProvider :theme="isDark ? 'dark' : 'light'">
      ...
      <VInput class="pt-0" hide-details="auto" :error-messages="errorMessages">
        <VCard
          ...
          class="ddtr-editor"
          :class="{ 'ddtr-editor--fullscreen': isFullscreen }"
        >
          ...
          <slot
            name="editor"
            v-bind="{ editor, props: { class: 'ddtr-editor__content', 'data-testid': 'value' } }"
          >
            <EditorContent
              class="ddtr-editor__content"
...
import { EditorTiptapOnChange } from '@/type'
...
interface Emits {
  (event: 'enter'): void
  (event: 'change', value: EditorTiptapOnChange): void
